...
/**
 * The persona switcher. A native `<select>` renders in the OS chrome — rounded, its own type, its
 * own highlight colour — which is the one thing on the page ADR 0012 cannot reach. This is the same
 * control drawn in the system: hairline box, mono meta, class carried by border treatment, and the
 * listbox keyboard contract (arrows, Home/End, Enter, Escape) written out by hand.
 */
export function PersonaSelect({ personas, persona, onPersona }: { personas: Persona[]; persona: Persona | null; onPersona: (p: Persona | null) => void }) {
  /** `null` is a real option — anonymous is a persona the demo uses, not an empty state. */
  ...
  // Pointer outside, or focus leaving the control entirely, closes it without choosing.
  useEffect(() => {
    if (!open) return;
  ...
  return (
    <div ref={root} className="relative flex items-center gap-3" onKeyDown={onKeyDown}>
      ...
      {open && (
        <ul
          ...
          className="absolute left-[86px] top-9 z-30 m-0 max-h-[60vh] w-[420px] list-none overflow-y-auto border border-[var(--ink)] bg-[var(--paper)] p-0"
        >
          {options.map((p, i) => {
            ...
            return (
              <li
                ...
                onClick={() => choose(i)}
                className={`rule-soft cursor-pointer px-3 py-2 first:border-t-0 ${active ? 'bg-[var(--paper-2)]' : ''}`}
              >
                <div className="flex items-center gap-2">
                  <span className="mono-xs w-3 shrink-0 text-[var(--ink)]">{selected ? '●' : ''}</span>
...
/** The 16px stroked chevron of `PanelToggle`, pointing the way the panel will move. */
