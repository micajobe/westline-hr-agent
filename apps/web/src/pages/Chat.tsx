...
export function ChatPage({
  ...
  // On a phone the three columns cannot coexist, so the rails start closed and open one at a time
  // over the full width. The chat is what the screen is for; it is never the thing that yields.
  const [narrowRail, setNarrowRail] = useState<'none' | 'list' | 'trace'>('none');
  ...
  const restored = Boolean(
    active && active.turns.length > 0 && !touched.current.has(active.chat_id),
  );
  ...
  useEffect(() => {
    if (!busy) saveChats(storage, chats);
  }, [chats, busy]);

  const updateChat = useCallback(
    (chat_id: string, patch: (c: Chat) => Chat) =>
      setChats((cs) => sortChats(cs.map((c) => (c.chat_id === chat_id ? patch(c) : c)))),
    [],
  );
  ...
  const patchTurn = useCallback(
    (chat_id: string, turn_id: string, patch: (t: Turn) => Turn) =>
      updateChat(chat_id, (c) => ({
        ...c,
        updated_at: Date.now(),
        turns: c.turns.map((t) => (t.turn_id === turn_id ? patch(t) : t)),
      })),
    [updateChat],
  );
  ...
  const send = useCallback(
    async (message: string, as: Persona | null, target: Chat) => {
        ...
        sortChats(
          cs.map((c) =>
            c.chat_id === chat_id
              ? {
                  ...c,
                  title: c.turns.length === 0 ? chatTitle(text) : c.title,
                  updated_at: Date.now(),
                  turns: [
                    ...c.turns,
                    { turn_id: tempId, persona: as, message: text, events: [], busy: true },
                  ],
                }
              : c,
          ),
      ...
      let realId = tempId;
      try {
        const env = await chatStream(
          {
            message: text,
            acting_person_id: as?.person_id ?? null,
            ...
            onTrace: (e) => {
              if (realId === tempId && e.turn_id) {
                ...
                updateChat(chat_id, (c) => ({
                  ...c,
                  turns: c.turns.map((t) =>
                    t.turn_id === tempId ? { ...t, turn_id: e.turn_id } : t,
                  ),
                }));
              }
        ...
        updateChat(chat_id, (c) => ({
          ...
          turns: c.turns.map((t) =>
            t.turn_id === realId
              ? { ...t, turn_id: env.turn_id, envelope: env, events: env.trace, busy: false }
              : t,
          ),
        }));
      } catch (err) {
        patchTurn(chat_id, realId, (t) => ({
          ...t,
  ...
  const decide = useCallback(
    async (chat_id: string, turn: Turn, decision: 'confirm' | 'cancel') => {
      const req = turn.envelope?.confirmation_required;
      if (!req || !turn.envelope) return;
      touched.current.add(chat_id);
      patchTurn(chat_id, turn.turn_id, (t) => ({ ...t, busy: true }));
      try {
        const env = await confirmStream(
          {
            conversation_id: turn.envelope.conversation_id,
          ...
          {
            onTrace: (e) =>
              patchTurn(chat_id, turn.turn_id, (t) =>
                t.events.some((x) => x.seq === e.seq) ? t : { ...t, events: [...t.events, e] },
              ),
          },
        );
        patchTurn(chat_id, turn.turn_id, (t) => ({
  ...
  const traceTurns: TraceTurn[] = useMemo(
    () =>
      turns.map((t) => ({
        turn_id: t.turn_id,
        label: `${t.persona?.name ?? 'anonymous'} — ${t.message}`,
        events: t.events,
        live: t.busy,
      })),
    [turns],
  ...
  const wideColumns = `${listCollapsed ? 'var(--rail-collapsed)' : 'var(--chat-list)'} minmax(0,1fr) ${traceCollapsed ? 'var(--rail-collapsed)' : 'var(--trace-rail)'}`;
  // One column, one child: exactly one of the list, the conversation, or the trace is mounted at
  // this width. Keeping all three mounted in 0px columns did not work — a zero-width grid item
  // does not clip its contents, so the rails painted straight over the conversation.
  const narrowColumns = 'minmax(0,1fr)';

  return (
    <>
      {narrow && (
        <div className="flex items-stretch border-b border-[var(--ink)]">
          {(['list', 'trace'] as const).map((rail) => (
            <button
              key={rail}
              type="button"
              aria-pressed={narrowRail === rail}
              onClick={() => setNarrowRail((r) => (r === rail ? 'none' : rail))}
              className={`flex-1 border-r border-[var(--ink)] px-4 py-3 text-left text-[length:var(--t-label)] uppercase tracking-[var(--track-label)] ${narrowRail === rail ? 'bg-[var(--ink)] text-[var(--paper)]' : 'text-[var(--muted)]'}`}
            >
              {rail === 'list' ? 'Chats' : 'Trace'}
            </button>
          ))}
        </div>
      )}
      <div
        ...
        style={{
          ...
          gridTemplateColumns: narrow ? narrowColumns : wideColumns,
        }}
      >
        {(!narrow || narrowRail === 'list') && (
          <ChatList
            chats={chats}
            activeId={active?.chat_id ?? null}
            collapsed={narrow ? false : listCollapsed}
            busy={busy}
            onToggle={() => (narrow ? setNarrowRail('none') : setListCollapsed((c) => !c))}
            onSelect={(id) => {
              selectChat(id);
              if (narrow) setNarrowRail('none');
            }}
            onNew={() => {
              onNew();
              if (narrow) setNarrowRail('none');
              ...
              <div className="mx-auto w-full max-w-[var(--chat-col)] px-6 py-8">
                {opening ? (
                  <div className="mt-6">
                    ...
                    <h1 className="headline text-[length:var(--t-h1)]">
                      Ask about Westline policy
                      <br />
                      as the person <em>you picked</em>.
                    </h1>
                    <p className="lede mt-6 max-w-[560px] text-[var(--muted)]">
                      Answers come only from the policy corpus and the HR data this persona is
                      allowed to see. Anything that would create or draft something pauses for your
                      confirmation first. The trace on the right shows every tool call as it
                      happens.
                    </p>
                    <div className="rule mt-8 pt-8">
                      <Composer
                        ...
                        onChange={setInput}
                        onSubmit={() => {
                          if (active) void send(input, persona, active);
                        }}
                      />
                    </div>
                    {tasks.length > 0 && (
                      <div className="rule mt-10 pt-5">
                        <Label className="mb-3">
                          Test scenarios · sets the persona and runs the question
                        </Label>
                        <ul className="m-0 list-none space-y-2 p-0">
                          {tasks.map((task, i) => (
                            <li key={task.id}>
                              ...
                              >
                                <span className="mono-xs uppercase text-[var(--muted)]">
                      ...
                      <li key={t.turn_id} className="space-y-5">
                        <div className="flex flex-col items-end gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            {t.persona && (
                              <ClassBadge workforce_class={t.persona.workforce_class} />
                            )}
                          </div>
                          <p className="m-0 max-w-[85%] whitespace-pre-wrap bg-[var(--paper-2)] px-4 py-3 text-[length:var(--t-body)]">
                            {t.message}
                          </p>
                        </div>
                        <div>
                          ...
                          {t.busy && !t.envelope && (
                            <p className="mono flex items-center gap-2 text-[var(--muted)]">
                              <Asterisk className="text-[var(--ink)]" />
                              {describeProgress(t.events)}
                            </p>
                          )}
                          {t.error && (
                            <p className="mono">
                          ...
                          {t.envelope && (
                            <div className="space-y-4">
                              ...
                              {t.envelope.confirmation_required && active && (
                                <ConfirmationCard
                                  request={t.envelope.confirmation_required}
                                  busy={t.busy}
                                  onDecision={(d) => void decide(active.chat_id, t, d)}
                                />
                              )}
                            </div>
                          )}
            ...
            {showDocked && (
              <div className="shrink-0 border-t border-[var(--ink)] bg-[var(--paper)]">
                <div className="mx-auto w-full max-w-[var(--chat-col)] px-6 py-4">
                  {citation && (
                    <div className="mb-4">
                      <CitationCard citation={citation} onClose={() => setCitation(null)} />
                    </div>
                  )}
                  {restored && (
                    <p className="mono-xs mb-3 text-[var(--muted)]">
                      {GLYPH.warn} Restored from this browser. The server keeps conversation context
                      only briefly, so treat a follow-up here as a fresh question.
                    </p>
                  )}
                  <Composer
                    variant="docked"
                  ...
                  <div className="mt-3 flex">
                    <button
                      type="button"
                      className="link link-muted ml-auto"
                      onClick={onNew}
                      disabled={busy}
                    >
                      New chat
        ...
        {(!narrow || narrowRail === 'trace') && (
          <TraceRail
            turns={traceTurns}
            collapsed={narrow ? false : traceCollapsed}
            onToggle={() => (narrow ? setNarrowRail('none') : setTraceCollapsed((c) => !c))}
          />
        )}
      </div>
    </>
...
function describeProgress(events: TraceEvent[]): string {
  ...
  if (last.type === 'tool_result' || last.type === 'retrieval')
    return `${last.tool?.replace('__', ' · ')} → ${last.result_summary ?? ''}`;
