...
export function DeskPage() {
  ...
  return (
    <main className="mx-auto max-w-[var(--max-content)] px-6 py-8">
      <h1 className="display text-[var(--text-40)] leading-none">The desk</h1>
      <p className="mt-2 max-w-[640px] text-[var(--muted)]">Mock tickets and drafts written by the two gated tools. Nothing here was sent anywhere. The store lives in <span className="mono">desk.sqlite</span> on the MCP service and resets on every redeploy.</p>
      {err && <p className="mt-6 text-[var(--accent)]">Desk unavailable: {err}</p>}
      {desk && (
        <>
          ...
          <section className="mt-10">
            ...
            <table className="w-full border-t border-[var(--rule)] text-left text-[var(--text-14)]">
              ...
              <tbody>
                {desk.drafts.map((d) => (
                  <tr key={d.draft_id} className="border-t border-[var(--rule)] align-top">
                    ...
                    <td className="py-2 pr-3"><Details summary={<span className="text-[var(--ink)]">{d.subject}</span>}><pre className="whitespace-pre-wrap font-[inherit] text-[var(--text-14)] leading-relaxed">{d.body}</pre></Details></td>
