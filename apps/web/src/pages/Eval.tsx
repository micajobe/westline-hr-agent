...
export function EvalPage() {
  ...
  useEffect(() => {
    api
      .evalLatest()
      .then((d) => {
        setData(d as EvalResults);
        setSource('latest');
      })
      .catch(() =>
        fetch('/sample-eval.json')
          .then((r) => r.json())
          .then((d) => {
            setData(d as EvalResults);
            setSource('sample');
    ...
    <main className="w-full max-w-[var(--max-content)] flex-1 overflow-y-auto px-6 py-8">
      ...
      {source === 'none' && (
        <p className="mt-6 text-[var(--muted)]">
          No results yet. Run <span className="mono">npm run eval -- --target local --runs 1</span>.
        </p>
      )}
      ...
      {data?.by_category && data.by_category.length > 0 && (
        <Table title="By category" rows={data.by_category} />
      )}
      {data?.ablations?.map((a) => (
        <Table
          key={a.name}
          title={`Ablation · ${a.name}${a.metric ? ` · ${a.metric}` : ''}`}
          rows={a.rows}
        />
      ))}
        ...
        <section className="mt-10 grid gap-8 md:grid-cols-2">
          {data.latency && (
            <div>
              ...
              <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-[length:var(--t-body)]">
                <dt className="text-[var(--muted)]">warm p50</dt>
                <dd className="mono">{ms(data.latency.warm_p50_ms)}</dd>
                <dt className="text-[var(--muted)]">warm p95</dt>
                <dd className="mono">{ms(data.latency.warm_p95_ms)}</dd>
                <dt className="text-[var(--muted)]">cold start</dt>
                <dd className="mono">
                  {data.latency.cold_runs?.length
                    ? data.latency.cold_runs.map(ms).join(' · ')
                    : ms(data.latency.cold_p50_ms)}
                </dd>
              </dl>
          ...
          {data.calibration && (
            <div>
              ...
              <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-[length:var(--t-body)]">
                <dt className="text-[var(--muted)]">exact agreement</dt>
                <dd className="mono">{pct(data.calibration.exact_agreement)}</dd>
                <dt className="text-[var(--muted)]">within ±1</dt>
                <dd className="mono">{pct(data.calibration.within_one_agreement)}</dd>
              </dl>
              {data.calibration.note && (
                <p className="mt-2 text-[var(--muted)]">{data.calibration.note}</p>
              )}
            </div>
          )}
        </section>
      ...
      {data && (
        <section className="mt-10">
          <details>
            <summary className="text-[var(--muted)]">raw json</summary>
            <div className="mt-2">
              <Json value={data} />
            </div>
          </details>
    ...
    <section className="mt-10">
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-t border-[var(--ink)] text-left text-[length:var(--t-body)]">
          ...
          <tbody>
            {rows.map((r, i) => (
              <tr key={i} className="border-t border-[var(--ink)]">
                {cols.map((c) => (
                  <td key={c} className={`py-2 pr-4 ${typeof r[c] === 'number' ? 'mono' : ''}`}>
                    {fmt(c, r[c] as number | string | null | undefined)}
                  </td>
                ))}
              </tr>
            ))}
...
function fmt(key: string, v: number | string | null | undefined): string {
  ...
  if (
    /pct|rate|precision|recall|accuracy|completion|agreement|match|groundedness|score/.test(key) &&
    v <= 1
  )
    return pct(v);
