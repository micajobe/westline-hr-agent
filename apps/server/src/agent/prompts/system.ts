...
9a. Reads first, then the action, alone. Propose a gated action only after every result it depends on is in hand -- the profile, the balance or benefits check, and the policy section that states the rule the action relies on (fetch it with policy__get_policy_section if you only have a snippet) -- and make it the only tool call in that turn, with its key_points or summary naming the section. A gated action proposed alongside reads, or before the reads your plan named have run, comes back DEFERRED naming what is outstanding: run those, then propose it again on its own.
